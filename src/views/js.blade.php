...
<script type="text/x-tmpl" id="preview_footer_{!! $dir !!}">
	...
	@if($overwrite)
        <button{!! $css_button !!} type="button" onclick="return TU{!! $dir_studly !!}.overwrite(this, {%=o.surpassId%});">{!! $button_label !!}</button>
    @else
        <button{!! $css_button !!} type="button" onclick="return TU{!! $dir_studly !!}.remove(this, {%=o.surpassId%});">{!! $button_label !!}</button>
    @endif
</script>
<script>
